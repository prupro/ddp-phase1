...
\section{Cooperation Policies} \label{sec:coop}
...
between $\mathcal{S}$ and $\mathcal{D}$ and cooperation distance between $\mathcal{S}$ and $\mathcal{R}$, respectively and $\alpha$ is pathloss exponent. This event E1 identifies whether an idle UE will be associated as a relay for the $k^{th}$ UE and participate in transmission.
